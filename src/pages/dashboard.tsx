...
import { getBillingDetails } from 'apiSdk/billing-details';
import { BillingDetailsInterface } from 'interfaces/billing-details';
import { getCars } from 'apiSdk/cars';
import { CarInterface } from 'interfaces/car';

import { BillingDetailsListPage as ShowCaseList } from 'pages/billing-details';
...
function DashboardPage() {
  ...
  const billingDetailsStat = useSWR<PaginatedInterface<BillingDetailsInterface>>(
    () => `/billing-details`,
    () => getBillingDetails({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: billingDetailsStat?.data?.totalCount,
      isLoading: billingDetailsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Cars',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`billing-details`} passHref legacyBehavior>
                  <Button
